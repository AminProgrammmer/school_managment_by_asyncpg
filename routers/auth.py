...
@router.post("/refresh")
async def refresh_token(response:Response,request:Request,db : asyncpg.pool.Pool = Depends(get_pg_conn)):
    ...
    return {"access_token": access_token, "token_type": "bearer"}
@router.post("/logout")
async def logout(request: Request,response : Response, db: asyncpg.pool.Pool = Depends(get_pg_conn)):
    cookie_token = request.cookies.get("refresh_token")
    if not cookie_token:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing refresh token")
    ...
    try:
        ...
        await db.execute("UPDATE refresh_tokens SET is_revoked = TRUE WHERE jti = $1", uuid.UUID(jti))
